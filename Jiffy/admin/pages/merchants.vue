<template>
    <v-layout>
        <v-container fluid>
                        ...
                        <v-tabs-items v-model="areatab">
                            <v-tab-item v-for="area in item.areas" :key="area.id">
                                <v-container fluid class="mb-10">
                                    <v-content>
                                        <v-data-iterator :items="merchants" :search="search" :page="page" :items-per-page.sync="itemsPerPage" hide-default-footer>
                                                ...
                                                <v-card flat style="min-height: 85vh; overflow:auto">
                                                    <v-list dense  three-line>
                                                        <v-list-item-group v-for="store in props.items" :key="store.id">
                                                            <v-list-item flat @click.stop="editedIndex=0,editedItem = store, actionSheet = true">
                                                                <v-list-item-avatar>
                                                                ...
                                                                <v-list-item-content>
                                                                    ...
                                                                    <v-list-item-subtitle>
                                                                        <small>{{ store.claimCode }}</small>
                                                                    </v-list-item-subtitle>
                                                                </v-list-item-content>
                                                                <v-list-item-icon>
                                                                    <v-chip :color="store.main ? 'warning' : 'info'" small> {{ store.main ? 'Branch' : 'Main' }} </v-chip>
                                                                    <v-chip dense :color="store.draft ? 'error' : 'success'" small> {{ store.draft ? 'Draft' : 'Live' }} </v-chip>
                                                                </v-list-item-icon>
                                                            </v-list-item>
                                                        </v-list-item-group>
                                                    </v-list>
            ...
            <v-dialog v-model="dialog" persistent :fullscreen="$isFullscreen()">
                        ...
                        <v-card>
                            ...
                            <v-card-text>
                                ...
                                <v-layout wrap>
                                    ...
                                    <v-flex xs3 sm3 md3>
                                        <v-text-field v-model.number="editedItem.rates.cr" type="number" label="CR" /> </v-flex>
                                    <v-flex xs3 sm3 md3>
                                        <v-text-field v-model.number="editedItem.rates.sc" type="number" label="SC" /> </v-flex>
                                    <v-flex xs3 sm3 md3>
                                        <v-text-field v-model="editedItem.delivery_time.open" type="time" label="Open" /> </v-flex>
                                    <v-flex xs3 sm3 md3>
                                        <v-text-field v-model="editedItem.delivery_time.close" type="time" label="Close" /> </v-flex>
                                    <v-flex xs12 sm12 md12>
                                        <v-autocomplete v-model="editedItem.tags" dense text :items="current_area.tags" filled chips color="blue-grey lighten-2" label="Tags" multiple>
                            ...
                            <v-card-actions>
                                <v-toolbar dense width="100%" max-width="100%">
                                    <v-spacer />
            ...
            <v-bottom-sheet v-model="coordiag" persistent :fullscreen="$isFullscreen()" max-width="400px">
                <v-card>
            ...
            <v-row justify="center">
                <v-dialog v-model="imagediag" max-width="400px" :fullscreen="$isFullscreen()">
                    <v-card>
                        <v-card-text>
                            <v-container>
                ...
                <v-list single-line dense>
                    ...
                    <v-list-item-group>
                        <v-list-item @click="selectedImages=[], editedIndex=0, editItem(editedItem)">
                            ...
                            <v-list-item-content>
                                <v-list-item-title>Edit Merchant</v-list-item-title>
                            </v-list-item-content>
                        </v-list-item>
                    </v-list-item-group>
                    <v-list-item-group>
                    ...
                    <v-list-item-group>
                        <v-list-item :to="'/menu/'+editedItem.id" target="_blank">
                            <v-list-item-icon>
                    ...
                    <v-list-item-group>
                        <v-list-item @click.stop="deleteDiag = true">
                            ...
                            <v-list-item-content>
                                <v-list-item-title>Delete Merchant</v-list-item-title>
                            </v-list-item-content>
                        </v-list-item>
                    </v-list-item-group>
                </v-list>
...
export default {
    ...
    computed: {
        ...
        merchants() {
            if (this.flag === 'All') return this.$store.getters['map/getStoresAsc']
            else if (this.flag === 'Draft') return this.$store.getters['map/getStoresAsc'].filter(x => x.draft === true)
            else return this.$store.getters['map/getStoresAsc'].filter(x => x.draft === false)
        },
        areas() {
            var areas = []
            var values = []
    ...
    methods: {
        ...
        deleteItem(id) {
            this.$fireStore.collection('foods').where('owner', 'array-contains', id).get().then(d => {
                d.docs.forEach(doc => {
                    this.$fireStore.collection('foods').doc(doc.id).update({
                        owner: this.$fireStoreObj.FieldValue.arrayRemove(id)
                    })
                })
            })
            this.$fireStore.collection('merchant').doc(id).delete()
            this.close()
        },
        editItem(item) {
            this.editedItem = 0
    ...
    mounted() {
        console.log(typeof this.$isFullscreen(), this.$isFullscreen())
        const firestore = this.$fireStoreObj()
