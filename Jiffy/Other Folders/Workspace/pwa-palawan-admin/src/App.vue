<template>
  <v-app height="100%" light id="e3" standalone>
    <v-app-bar app small fixed elevate-on-scroll dense color="blue">
      <v-app-bar-nav-icon small color="white" @click.stop="drawer = !drawer"></v-app-bar-nav-icon>
      ...
      <v-list dense dark>
        <v-list-item two-line>
          ...
          <v-list-item-content>
            ...
            <v-list-item-subtitle>Admin System</v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
        <v-divider></v-divider>
        <v-list-item link to="/">
        ...
        <v-list-group>
          ...
          <v-list-item class="ml-2" v-for="(admin, i) in admintools" :key="i" link :to="admin.url">
            <v-list-item-title class="white--text">{{admin.title}}</v-list-item-title>
          </v-list-item>
        </v-list-group>
        <v-list-group>
          <template v-slot:activator>
          ...
          <v-list-item class="ml-2" v-for="(admin, i) in finance" :key="i" link :to="admin.url">
            <v-list-item-title class="white--text">{{admin.title}}</v-list-item-title>
          </v-list-item>
        </v-list-group>
...
export default {
  ...
  data: () => ({
    ...
    countries: [],
    items: [
      { title: 'Van Booking', icon: 'fas fa-shuttle-van', url: '/transfers' },
      { title: 'Bookings', icon: 'fas fa-shuttle-van', url: '/bookings' },
    ],
    finance: [
      { title: 'Cash Management', icon: 'far fa-calendar-check', url: '/cash-management' },
      { title: 'Credits', icon: 'far fa-calendar-check', url: '/agent-credits' },
      { title: 'Billing', icon: 'far fa-calendar-check', url: '/billing' },
      { title: 'Obligations', icon: 'far fa-calendar-check', url: '/obligations' }
    ],
    admintools: [{ title: 'Rate Management', icon: 'far fa-calendar-check', url: '/rate-editor' },
    { title: 'Package Editor', icon: 'far fa-calendar-check', url: '/package-editor' },
     { title: 'Master File Editor', icon: 'far fa-calendar-check', url: '/masterfile-editor' },
      { title: 'Drivers', icon: 'far fa-calendar-check', url: '/drivers' },
      { title: 'Suppliers', icon: 'far fa-calendar-check', url: '/suppliers' },
      { title: 'Partners', icon: 'far fa-calendar-check', url: '/partners' },
      { title: 'Marketing', icon: 'far fa-calendar-check', url: '/marketing' }
    ],
    sheet: false,
    tiles: [
  ...
  mounted() {
    ...
    this.loading = true
    this.fetchServices()
    fetch('https://api.diskubrepalawan.com/get-partners')
      .then(res => res.json())
      .then(res => {
        this.$store.commit('updatePartners', res)
      })
      .catch(err => {
        console.log(err)
      })
