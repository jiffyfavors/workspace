              ...
              <v-card class="mx-auto" width="500" min-height="100%" max-width="100%">
                ...
                <v-layout row wrap mx-5 pt-5>
                  <v-flex xs12 sm12 md12>
                    <v-autocomplete v-model="partner" :items="items" :loading="isLoading" :search-input.sync="search" item-value="id" item-text="business_name" label="Select Partner" placeholder="Start typing to Search" prepend-icon="fas fa-search" return-object></v-autocomplete>
                  </v-flex>
                  <template v-if="partner">
                    ...
                    <v-flex xs12 sm12 md12>
                      <v-radio-group :mandatory="true"  v-if="services.length>0" dense v-model="selectedService">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['getPartners','contactSheet', 'getTransResult', 'getSearchData', 'getServices', 'getAreas']),
