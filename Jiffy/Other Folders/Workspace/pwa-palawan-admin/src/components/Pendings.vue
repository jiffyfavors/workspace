<template>
     ...
     <v-card min-height="100%">
        ...
        <small>Booking for Approval</small>
              ...
              <v-card class="pa-1">
                ...
                <v-list-item @click="isexpand = !isExpanded(item), expand(item, isexpand) ">
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle><small>{{getSource(item.src)}}</small></v-list-item-subtitle>
...
export default {
  ...
  methods: {
    ...
    fetchBookings() {
      fetch('https://www.diskubrepalawan.com/index_dev.php/get-ds-pendings').then((data) => {
        return data.json()
      }).then((data) => {
        this.items = data
