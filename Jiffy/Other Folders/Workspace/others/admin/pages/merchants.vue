 ...
 <v-layout>
                    ...
                    <v-tab-item v-for="item in areas" :key="item.id">
                        <v-card color="basil" flat>
                            ...
                            <v-tab-item v-for="area in item.areas" :key="area.id">
                                <v-data-iterator :items="merchants" :search="search" :items-per-page=10 hide-default-footer>
                                    <template v-slot:header>
                                        <v-toolbar dense color="success" class="mb-1">
                                            <v-text-field solo dense v-model="search" append-outer-icon="mdi-magnify" clearable hide-details label="Search"></v-text-field>
                                            <v-btn icon  color="blue" @click.stop="showMerchant()" ><v-icon>mdi-refresh</v-icon></v-btn>
                                              <v-btn icon  color="blue" @click.stop="dialog =true"><v-icon>mdi-plus</v-icon></v-btn>
                                        </v-toolbar>
                                    </template>
        ...
        <v-dialog v-model="dialog" persistent max-width="500px">
            <v-card class="mx-auto" max-width="400">
                <v-card-title> <span class="title font-weight-light">{{ formTitle }}</span> </v-card-title>
                <v-card-text>
                    ...
                    <v-layout wrap>
                        <v-flex xs12 sm12 md12>
                            <v-select v-model="editedItem.tags" :items="current_area.tags" label="Select Option" multiple>
                                <template v-slot:selection="{ item, index }">
                                    <v-chip small dense color="blue" small close @click:close="editedItem.tags.splice(editedItem.tags.indexOf(item),1)"> <span>{{ item }}</span> </v-chip>
                                </template>
                            </v-select>
                        </v-flex>
                    </v-layout>
        ...
        <v-bottom-sheet inset v-model="actionSheet">
            <v-list single-line dense>
                ...
                <n-link :to="'/menu/'+editedItem.id" prefetch>
                    <v-list-item-group>
                        <v-list-item>
                            <v-list-item-icon>
...
export default {
    ...
    computed: {
        ...
        merchants() {
            return this.$store.getters['map/getStoresAsc']
        },
        areas() {
            var areas = []
            var values = []
    ...
    methods: {
        ...
        deleteItem(id) {
            var ref = this.$fireStore.collection('merchant').doc(id)
