                    ...
                    <v-card color="basil" flat>
                        ...
                        <v-tabs-items v-model="areatab">
                            <v-tab-item v-for="area in item.areas" :key="area.id">
                                <v-data-iterator :items="merchants" :search="search" :items-per-page=50>
                ...
                <v-card-text>
                    ...
                    <v-layout wrap>
                        <v-flex xs12 sm12 md12>
                            <v-select v-model="editedItem.tags" :items="tags" label="Select Option" multiple>
                                <template v-slot:selection="{ item, index }">
                                    <v-chip small dense color="blue" small close @click:close="editedItem.tags.splice(editedItem.tags.indexOf(item),1)">
                                        <span>{{ item }}</span>
                                    </v-chip>
                                </template>
                            </v-select>
...
export default {
    ...
    computed: {
        tags() {
            return ['Market', 'Groceries', 'Pharmacy']
        },
        numberOfPages() {
            return Math.ceil(this.merchants.length / this.itemsPerPage)
        },
        formTitle() {
            return this.editedIndex === -1 ? 'New Shop' : 'Edit Shop Info'
        },
        current_area() {
            return this.$store.getters['admin/getCurrentArea']
        },
        merchants() {
            return this.$store.getters['map/getShopsAsc']
        },
        areas() {
            var areas = []
            var values = []
    ...
    methods: {
        deleteItem(id) {
            ...
            //     })
            // })
            this.$fireStore.collection('shops').doc(id).delete()
            this.close()
        },
        editItem(item) {
            this.editedItem = 0
        ...
        showMerchant() {
            ...
            this.isLoading = true
            this.$store.dispatch('map/getShopsByArea', {
