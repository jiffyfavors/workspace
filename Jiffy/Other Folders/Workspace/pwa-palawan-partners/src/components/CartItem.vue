            ...
            <v-form ref="bookForm" lazy-validation @submit="bookingSubmit">
              ...
              <v-list two-line subheader>
                ...
                <v-list-item>
                  <v-layout wrap>
                    <v-flex xs12 md12 sm12>
                      <v-text-field required hint="Input Zero(0) if you want to charge to your bill or you will pay on the later date" v-model="collect" type="number" label="Collection Advise" placeholder="Collection Advise"></v-text-field>
                    </v-flex>
                    <v-flex xs12 md12 sm12>
...
export default {
 ...
 this.data.rates.extra.fees.forEach(a=>{
  this.price = this.price + (parseFloat(a.rate) * parseInt(this.data.pax))
 })

    this.collect = this.price
  ...
  methods: {
    setColor() {
      ...
      if (parseFloat(this.collect) > 0) {
        payment.push({
          mode: 'COLLECT',
          amount: this.$options.filters.toPHP(this.collect).replace(/^\D+/g, '').replace(',', ''),
