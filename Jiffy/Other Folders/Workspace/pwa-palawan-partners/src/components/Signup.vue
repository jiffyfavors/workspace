...
export default {
  ...
  methods: {
    async postData(path = [], data = {}) {
      ...
      var api = 'https://api.diskubrepalawan.com/index_dev.php'
      path.forEach(a => {
        api = api + '/' + a
      })
    ...
    registerNow: function() {
      ...
      var restrict = []
      if (!allow)
        restrict = ['/tours', '/packages']

      console.log(this.type)
      var ref = this.type.code + '-' + this.country.code + '-' + result
      var business = {
        id: ref,
      ...
      this.postData(['auth', 'new-partner'], { business: business, user: user })
          .then((data) => {
              ...
              if(this.resData.code===200)
               this.loginMe({username:this.email, password: this.pass1})
    ...
    loginMe(user){
         ...
         this.isLoading = true
      var auth = { username: user.username, password: user.password }
      this.loadingdiag = true
    ...
    loginForm: function(e) {
     this.loginMe({username:this.username, password: this.password})
    }
  }
};
</script>
