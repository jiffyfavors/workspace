export default ({ app, store }) => {
  ...
  const setTokenAndSubscribe = (token) => {
    let areas = store.getters['rider/getArea']
    app.$fireStoreObj().collection('tokens').doc('riders').collection('tokens').doc(areas).get().then((snap) => {
      topicSubscription(
        [token],
        ['riders', areas + '_rider'], 'subscribeToTopic')
      store.commit('auth/setFCMToken', token)
      if (snap.exists) {
        let index = snap.data().tokens.indexOf(token)
        if (index < 0) {
          app.$fireStoreObj().collection('tokens').doc('riders').collection('tokens').doc(areas).update({
            tokens: app.$fireStoreObj.FieldValue.arrayUnion(token)
          })
        }
      } else {
        app.$fireStoreObj().collection('tokens').doc('riders').collection('tokens').doc(areas).set({
          tokens: [token]
        })
      }
    })
