        ...
        <template #extension>
          <client-only>
            <v-tabs v-if="!isHide" v-model="currentItem" slider-color="white" dense grow show-arrows center-active centered>
              ...
              <v-tab v-for="item in initialTab" :key="item.name" @click.stop="$store.commit('cart/updateCategory', item)" :href="'#tab-' + item.name"> {{ item.name }} </v-tab>
...
export default {
      ...
      if (fare !== null && fare.hasOwnProperty('tags')) {
        ...
        tags.forEach(tag => {
          ...
          if (foods.length > 0) menu.push({
           ...
           this.$nextTick(() => { this.addItem(this.$store.getters['cart/getCurrentCategory']);});
        return menu
      } else {
        return []
      }
  ...
  mounted() {
    ...
    let loc = this.$store.getters['map/getLocation'];
    if (loc != null) {
